...
function productOfNonZeroes(num:number):number{
    let product = 1;
    while(num > 0){
        let digit = num % 10;
        if(digit != 0){
...
function findDigitDifference(num: number):number{
    const numStr = num.toString();
    let maxDigit = parseInt(numStr[0]);
    let minDigit = parseInt(numStr[0]);
